...
namespace ExampleLibrary
{
    ...
    new LinearAxis() { Key = "x_axis", AbsoluteMinimum = x0, AbsoluteMaximum = x1, Position = AxisPosition.Top });
        ...
        [Example("#919: Issues with LinearAxes with AxisPosition.None")]
        public static PlotModel AxisVisbilityWhenPositionIsNone()
        {
            var plot = new PlotModel() { Title = "LinearAxes with AxisPosition.None", Subtitle = "Click to toggle the title of the (visible) X axis." };
            Axis xaxis = new LinearAxis { Position = AxisPosition.None, IsAxisVisible = true, Key = "X" };
            plot.Axes.Add(xaxis);
            plot.Axes.Add(new LinearAxis { Position = AxisPosition.None, IsAxisVisible = false, Key = "Y" });

            plot.Series.Add(new FunctionSeries(x => x * x, 0, 1, 0.001) { Color = OxyColors.Blue });
            plot.Series.Add(new FunctionSeries(x => x * x, 0, 1, 0.001) { XAxisKey = "X", YAxisKey = "Y", Color = OxyColors.Red });

            plot.Annotations.Add(new TextAnnotation() { Text = "The well-formed axes are auto-generated by OxyPlot\nThe ticks on the left are residue from a LinearAxis\nwith Position = AxisPosition.None", TextPosition = new DataPoint(0.5, 0.5), FontSize = 20 });

            plot.MouseDown += (s, e) =>
            {
                // when the title is non-null, the renderer throws an unhelpful exception
                xaxis.Title = xaxis.Title == null ? "Title" : null;
